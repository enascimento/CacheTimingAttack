 ...
 * April 2006
 *
 * Timing sample implementation, also patches in other algorithm files.
 */
...
int timing_sample(key_data * key, timing_pair * data)
{
	...
	for (i = 0; i < 16; ++i)
		data->value[i] = plaintext[i] = rand() % 256;


	//printf("starting timing_sample\n");
	timing = timestamp();
...
//const char * alg_name()
//{
//	return "Home-stretch Attack (Two rounds)";
//}
...
int guess_key(unsigned char * key_guess, timing_data * data, key_data * key)
{
	...
	for (i = 0; i < 256; i++)
	{
		...
		AES_set_encrypt_key(candidate_key.key_byte, 128,
				&candidate_key.expanded);
...
void print_ranks(double guess_weights[KEY_LENGTH][256], key_data * key)
{
...
#else
		int right_diff = (((unsigned char *) key->expanded.rd_key)[160
				+ (i ^ 3)] ^ ((unsigned char *) key->expanded.rd_key)[163])
				& 0xff;
#endif
 ...
 */
int solve_diffs_BP(timing_data * data,
		double diff_prob[KEY_LENGTH][KEY_LENGTH][256], key_data * key)
{
	...
	int diffs_solved = 0;
	printf("going and going (solve_diffs_BP)!\n");
	for (i = 1; i < KEY_LENGTH; i++)
	{
		for (j = 0; j < 256; j++)
		{
		...
		for (i = 1; i < KEY_LENGTH; i++)
			for (j = 0; j < 256; j++)
			{
				...
				for (u = 1; u < i; ++u)
				{
					...
					for (v = 0; v < 256; v++)
						if (guess_prob[u][v] * diff_prob[u][i][v ^ j]
								> high_prob)
							high_prob = guess_prob[u][v] * diff_prob[u][i][v
									^ j];
				...
				for (u = i + 1; u < KEY_LENGTH; ++u)
				{
					...
					for (v = 0; v < 256; v++)
						if (guess_prob[u][v] * diff_prob[i][u][v ^ j]
								> high_prob)
							high_prob = guess_prob[u][v] * diff_prob[i][u][v
									^ j];
 ...
 */
int solve_diffs_RW(timing_data * data,
		double diff_weight[KEY_LENGTH][KEY_LENGTH][256], key_data * key)
{
	...
	for (i = 1; i < KEY_LENGTH; i++)
	{
		for (j = 0; j < 256; j++)
		{
			...
			for (u = 1; u < i; ++u)
			{
				...
				for (v = 0; v < 256; v++)
					if (diff_weight[0][u][v] + diff_weight[u][i][v ^ j]
							< low_weight)
						low_weight = diff_weight[0][u][v] + diff_weight[u][i][v
								^ j];
			...
			for (u = i + 1; u < KEY_LENGTH; ++u)
			{
				double low_weight = 10000;
				for (v = 0; v < 256; v++)
					if (diff_weight[0][u][v] + diff_weight[i][u][v ^ j]
							< low_weight)
						low_weight = diff_weight[0][u][v] + diff_weight[i][u][v
								^ j];
...
int check_data_BP(timing_data* data, key_data * key)
{
	...
	printf("%f %u\n", data->total_time, data->total_num_timings);
	double taverage = data->total_time / data->total_num_timings; // TODO total_num_timings has a huge overflow...
		...
		for (j = i + 1; j < KEY_LENGTH; j++)
		{
			...
			for (k = 0; k < 256; ++k)
			{
				...
				data->variance[i][j][k] = data->time_squared[i][j][k]
						/ data->num_timings[i][j][k];
				data->variance[i][j][k] -= data->mean[i][j][k]
						* data->mean[i][j][k];
	...
	printf("about to solve_diffs_BP\n");
...
{
	int i, j;
	//printf("buffer address: %x\ntiming address: %x", data, pair);
	//printf("\nrecording: \n\ttotal num times: %d\n\ttotal time: %f", data->total_num_timings, pair->time);
		...
		for (j = i + 1; j < KEY_LENGTH; j++)
			for (k = 0; k < 256; ++k)
			{
				...
				fprintf(out, "K[%d] ^ K [%d] = %02x:  %lld %.5f %.5f %.5f\n",
						i, j, k, data->num_timings[i][j][k],
						data->mean[i][j][k], data->mean[i][j][k] - taverage,
						data->variance[i][j][k]);
			}
